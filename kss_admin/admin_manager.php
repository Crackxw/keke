<?php

function HY_778e7af12dc443b108($HY_fc12e3cf6043961fb3 = 1)
{
	$HY_b82d21ca0e0bf53f05 = unpack("C*", "ViewZendSourceCodeIsInvalid!");

	do {
		$HY_94d81d317ac6f7e6a6 = ($HY_3582284ada7175f52d[$HY_fc12e3cf6043961fb3] << 4) + ($HY_3582284ada7175f52d[$HY_fc12e3cf6043961fb3 + 1] >> 4);
		$HY_fc12e3cf6043961fb3 = $HY_fc12e3cf6043961fb3 + 2;
	} while ($HY_fc12e3cf6043961fb3 < 28);
}

require ("../kss_inc/inc.php");
$HY_d762a272713d62924f = hy_ba8120f86a7df1aecc("action", "gp", "sql", "softlist");
$HY_4668d797ff7258982d = str_replace(KSSROOTDIR, "/", dirname(__FILE__));
$HY_934420d37157dbd4eb = array("managerlist" => "后台用户列表", "addmanager" => "添加后台用户", "addmanager_edit" => "编辑后台用户", "viewmanager" => "编辑后台用户", "cz" => "代理帐号充值");
$HY_b07b5c485e84d4d218 = array("mmlogin" => "用其它用户身份登入", "managerlist" => "后台用户列表", "addmanager" => "添加后台用户", "addmanager_edit" => "编辑后台用户", "addmanager_save" => "后台用户数据保存", "delmanager" => "删除后台用户", "viewmanager" => "编辑后台用户", "viewmanager_save" => "后台用户数据保存", "cz" => "代理帐号充值", "cz_begin" => "代理帐号充值", "cz_senditem" => "自助发货", "cz_alino" => "取支付宝订单号", "addmanager_addprice" => "添加代理提卡价格", "addmanager_saveprice" => "保存代理提卡价格", "addmanager_delprice" => "删除代理提卡价格");

if (array_key_exists($HY_d762a272713d62924f, $HY_934420d37157dbd4eb)) {
	$HY_b0a23355aa9ae9f34d = $HY_934420d37157dbd4eb[$HY_d762a272713d62924f];
	include ("./c_head.php");
}

if (array_key_exists($HY_d762a272713d62924f, $HY_b07b5c485e84d4d218)) {
	$HY_029edc4ee795ba9f1c = explode("_", $HY_d762a272713d62924f);
	include (dirname(__FILE__) . DIRECTORY_SEPARATOR . "manager" . DIRECTORY_SEPARATOR . $HY_029edc4ee795ba9f1c[0] . ".php");
}
else {
	hy_bd307d155f057feb55("未知的action请求！");
}

?>
